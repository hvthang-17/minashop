              ...
              <tr>
                <td>
                  <div class="shopping-cart__product-item">
                    <img loading="lazy" src="{{ $item->model ? asset('uploads/products/thumbnails/' . $item->model->image) : asset('images/default.png') }}" width="120" height="120" alt="{{$item->name}}" />
                  </div>
                </td>
                <td>
          ...
          <div class="cart-table-footer">
            ...
            @if(!Session::has('coupon'))
              <form action="{{route('cart.coupon.apply')}}" method="POST" class="position-relative bg-body">
                ...
                <input class="btn-link fw-medium position-absolute top-0 end-0 h-100 px-4" type="submit" value="ÁP DỤNG MÃ GIẢM GIÁ">
              </form>
            @else
              <form action="{{route('cart.coupon.remove')}}" method="POST" class="position-relative bg-body">
                @csrf
                @method('DELETE')
                <input class="form-control" type="text" name="coupon_code" placeholder="Mã giảm giá" value="@if(Session::has('coupon')) {{Session::get('coupon')['code']}} Đã áp dụng! @endif">
                <input class="btn-link fw-medium position-absolute top-0 end-0 h-100 px-4" type="submit" value="XÓA MÃ GIẢM GIÁ">
              </form>
            @endif
            <form action="{{route('cart.empty')}}" method="POST" class="mt-2">
              @csrf
              @method('DELETE')
        ...
        @else
            <div class="row">
                <div class="col-md-12 text-center pt-5 bp-5">
                    ...
                    <a href="{{route('shop.index')}}" class = "btn btn-light">Mua ngay</a>
                </div>
            </div>
        @endif
      </div>
